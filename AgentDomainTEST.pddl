...
(:action investigate-room :parameters (?a ?room)
:precondition (and (ROOM ?room) (AGENT ?a)
                   (alive ?a) (not (scared ?a))
                   (in-room ?a ?room)
                   (not (died ?a)) 
               )
:effect (and (explored-room ?a ?room)
         )
 )
...
(:action neutralize-killer :parameters (?a ?k ?place)
...
(:action nothing-to-do :parameters (?a1)
