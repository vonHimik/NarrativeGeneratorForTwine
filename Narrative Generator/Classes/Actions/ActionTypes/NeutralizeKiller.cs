...
namespace Narrative_Generator
{
    class NeutralizeKiller : PlanAction
    {
        ...
        public NeutralizeKiller(ref Agent agent, ref Agent killer, ref Location location)
        {
            Arguments.Add(agent);
        ...
        public override bool CheckPreconditions(WorldBeliefs state)
        {
            return Agent.GetRole() == "usual" && Agent.GetStatus() && Killer.GetRole() == "killer" && Killer.GetStatus()
                      && Location.SearchAgent(Agent) && Location.SearchAgent(Killer) && Agent.GetObjectOfAngry().AngryCheckAtAgent(Killer);
        }

        public override void ApplyEffects(WorldBeliefs state)
        {
            Killer.SetStatus(false);
        }
